function [rate_4cm_3D, raster_data, cluster_id_good] = process_spike_data( ...
    KiloFolder, interval_data, occupancy_4cm, halls, trans_beg, trans_end, maxRHD_ts, varargin)
% PROCESS_SPIKE_DATA
...
addParameter(p, 'BinEdges4cm', 0:4:534, @(v) isvector(v) && isnumeric(v) && numel(v) >= 2 && issorted(v));
parse(p, varargin{:});
opt = p.Results;
...
try
    ...
    for pth = 1:numel(path_parts)
        if ~isempty(regexp(path_parts{pth}, '^VR\d+$', 'once'))
            animal_name = path_parts{pth};
            break;
        end
    end
catch
...
for u = 1:nUnits
    ...
    for tr = 1:nTrials
        ...
        idx = interp1(ts_RS, (1:N)', ts_tr, 'nearest', 'extrap');
        if isempty(idx) || all(~isfinite(idx))
            rate = zeros(1, nBins);
            rate(~isfinite(occ_row)) = NaN;
            rate_4cm_3D(tr, :, u) = rate;
            continue;
