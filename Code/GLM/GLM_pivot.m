...
1%% GLM_unpack_restrict_long_flags.m
% Unpacks GLM coefficients/p-values into separate columns,
% allows selection of which MasterTable to trim,
% keeps only relevant metadata,
% and adds sign/significance flags and split beta columns for pivot analysis.
...
if ismember('Block_Filter', MT.Properties.VariableNames)
    ...
    fprintf('Excluded %d units failing Block_Filter test (%.1f%% of total)\n\n', ...
...
for blk = blocks
    for p = 1:numel(predictors)
        ...
        if ismember(beta_col, MT_reduced.Properties.VariableNames)
            ...
            % --- Add sign/significance flags
            Ttemp.isSig  = Ttemp.Pval < 0.05;
            Ttemp.isPos  = Ttemp.Beta > 0;
            Ttemp.isNeg  = Ttemp.Beta < 0;

            % --- Split beta into positive/negative columns for pivoting
            Ttemp.Beta_Pos = Ttemp.Beta .* (Ttemp.Beta > 0);
            Ttemp.Beta_Neg = Ttemp.Beta .* (Ttemp.Beta < 0);

            Long = [Long; Ttemp]; %#ok<AGROW>
        end
    end
end
...
out_long    = fullfile(matpath, sprintf('%s_%s_GLMLong_flags.mat', base, vars{idx}));

save(out_reduced, 'MT_reduced', '-v7.3');
save(out_long, 'Long', '-v7.3');

fprintf('\nSaved trimmed and long-format tables with flags:\n');
fprintf('  Reduced: %s\n', out_reduced);
fprintf('  Long:    %s\n', out_long);
