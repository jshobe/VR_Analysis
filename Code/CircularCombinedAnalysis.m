...
% Figure 1 left subplot: mean speed before site
speedWindow_cm = 20;             % average speed in final 20 cm before site

% Figure 1 right subplot: raw deceleration
decelPreWindow_cm     = 5;       % near-site window for deceleration
decelControlWindow_cm = [30 40]; % control window before same site
...
requireLickNearReward = menu( ...
    'Exclude target trials without a lick near reward delivery?', ...
    'No', ...
    'Yes');

if isequal(requireLickNearReward, 0)
    error('No lick-near-reward option selected.');
end

lickNearRewardEnabled = (requireLickNearReward == 2);
...
disp('Means by state:')
disp(groupsummary(T, 'state', 'mean', {'approachSpeed','decelNearSpeed','decelControlSpeed','rawDecel'}))

disp('Means by site and state:')
disp(groupsummary(T, {'siteDeg','state'}, 'mean', {'approachSpeed','decelNearSpeed','decelControlSpeed','rawDecel'}))
...
sessionTitle = sprintf('%s | %s | %s', sessionTitle, lapLabel, lickFilterLabel);
...
ylabel(sprintf('Mean speed in final %.0f cm before site (cm/s)', speedWindow_cm), ...
...
ylabel(sprintf('Raw deceleration: %.0f-%.0f cm speed minus final %.0f cm speed (cm/s)', ...
    decelControlWindow_cm(1), decelControlWindow_cm(2), decelPreWindow_cm), ...
    'FontSize', labelFontSize);
...
for s = 1:numel(siteOrder)
    ...
    for st = 1:numel(stateOrder_profile)

        idx = allProfileSiteDeg == thisSiteDeg & strcmp(allProfileState, stateOrder_profile{st});
        ...
        h = plot(profileCenters_deg, meanProfile, 'LineWidth', 2);
        plotHandles(end+1) = h; %#ok<SAGROW>
        plotLabels{end+1} = stateLabels_profile{st}; %#ok<SAGROW>
    end
...
function [approachSpeed, decelNearSpeed, decelControlSpeed, rawDecel] = get_event_metrics( ...
    ...
    % Mean speed window for Figure 1 left subplot
    speedMask = pos_cm_unwrapped >= (cp - speedWindow_cm) & ...
                pos_cm_unwrapped <  cp;
    speedMask = speedMask & beforeMask;

    % Near-site speed window for raw deceleration
    decelNearMask = pos_cm_unwrapped >= (cp - decelPreWindow_cm) & ...
                    pos_cm_unwrapped <  cp;
    decelNearMask = decelNearMask & beforeMask;

    % Control speed window for raw deceleration
    ...
    if isempty(speedVals)
        ...
        approachSpeed = mean(speedVals);
    end

    if isempty(decelNearVals)
        decelNearSpeed = NaN;
    else
        decelNearSpeed = mean(decelNearVals);
    end

    if isempty(decelControlVals)
        decelControlSpeed = NaN;
    else
        decelControlSpeed = mean(decelControlVals);
    end
