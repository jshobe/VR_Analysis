...
%   'SpeedThresh'      : 2.5 - speed gate in cm/s (used in behavior + spikes)
%   'MakeLegacyPlots'  : true - compute/load legacy caches; skip legacy plotting when false
%
% Notes:
%   - V2 FSC + 3D 4cm analysis is invoked per region via run_spatial_analysis_v2.
%   - Ensure run_spatial_analysis_v2.m and process_region.m are on your MATLAB path.
%   - Manual cache workflow: caches are not auto-invalidated on SpeedThresh changes; a warning is logged.
    ...
    log_msg('SpeedThresh: %.3f cm/s', opts.SpeedThresh);
    ...
    for r = 1:numel(opts.Regions)
        ...
        % process_region will compute/load caches and skip plotting if opts.MakeLegacyPlots=false
        ...
        try
            ...
            % Pass your Blocks, plotting overrides, speed, and optionally disable V2 plots to save time
            run_spatial_analysis_v2(animal_folder, region_folder, out_folder_v2, ...
                ...
                'SpeedThresh', opts.SpeedThresh, ...
                'DoPlots', false);  % default OFF to avoid plot time; set true when needed
