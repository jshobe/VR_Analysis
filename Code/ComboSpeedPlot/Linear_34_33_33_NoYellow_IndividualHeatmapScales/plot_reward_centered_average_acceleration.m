...
if G.outlierFilterApplied
    filterText = sprintf( ...
        ' | robust outliers omitted: %d (%.3g%%)', ...
        G.nOutliersRemoved, ...
        100 * G.outlierFraction);
else
    filterText = '';
end

title(ax, { ...
    'Signed acceleration by lap group'; ...
    sprintf(['a = v dv/dx | quadratic fit, %d bins (~%.0f cm)' ...
        '%s'], ...
        cfg.spatialAccelerationWindowBins, windowCm, filterText)}, ...
    'FontWeight', 'normal', ...
...
% Force all lap-group entries onto one horizontal row when supported.
try
    lgd.NumColumns = G.nGroups;
catch
end
...
S.nAccelerationOutliersRemoved = G.nOutliersRemoved;
S.accelerationOutlierFraction = G.outlierFraction;
S.accelerationOutlierMask = G.outlierMask;
S.filteredAccelerationMatrix = G.filteredAccelMat;
