...
%   middle: overall speed +/- SEM with matching 25-lap speed curves
%   bottom: spatially derived signed acceleration in 25-lap blocks
%
...
if ~isfield(cfg, 'accelerationLapBlockSize')
    cfg.accelerationLapBlockSize = 25;
end
...
%% Validated reward-artifact correction
% The earlier circular analysis removed the reward-onset speed sample and
% the immediately following sample, then interpolated across those targeted
% values. Keep this correction enabled by default for both displayed speed
% and the unthresholded speed used to derive acceleration.
if ~isfield(cfg, 'correctRewardSpeedArtifact')
    cfg.correctRewardSpeedArtifact = true;
end
...
% Run invariant checks for every selected file and print a brief audit.
if ~isfield(cfg, 'auditRewardSpeedArtifact')
    cfg.auditRewardSpeedArtifact = true;
end
...
for f = 1:nFiles
    ...
    %% Window-based speed and reward-artifact correction
    % Calculate speed first, then apply the validated reward-onset
    % correction before either plotting or spatial acceleration.
    D.speed_cm_s_uncorrected = compute_window_speed( ...
        D.t, ...
        D.pos_cm_unwrapped, ...
        cfg.speedWindow_cm_forPlot);

    if cfg.correctRewardSpeedArtifact
        [D.speed_cm_s_unthresholded, rewardArtifactAudit] = ...
            correct_reward_speed_artifact( ...
                D.t, ...
                D.reward, ...
                D.speed_cm_s_uncorrected, ...
                cfg.rewardArtifactSamplesAfterOnset);
    else
        D.speed_cm_s_unthresholded = D.speed_cm_s_uncorrected;

        rewardArtifactAudit = struct( ...
            'nRewardOnsets', sum(diff([false; D.reward(:) > 0]) == 1), ...
            'nTargetSamples', 0, ...
            'nInterpolated', 0, ...
            'nRemainingNaN', 0, ...
            'nonTargetChangedCount', 0, ...
            'onlyTargetSamplesChanged', true, ...
            'passed', true);
    end

    if cfg.auditRewardSpeedArtifact
        if ~rewardArtifactAudit.passed || ...
                rewardArtifactAudit.nonTargetChangedCount ~= 0
            error([ ...
                'Reward-artifact QC failed for %s: %d non-target ' ...
                'speed samples changed.'], ...
                file{f}, ...
                rewardArtifactAudit.nonTargetChangedCount);
        end

        fprintf([ ...
            'Reward-artifact audit %s: %d onsets, %d targeted samples, ' ...
            '%d interpolated, %d left NaN, 0 non-target changes.\n'], ...
            file{f}, ...
            rewardArtifactAudit.nRewardOnsets, ...
            rewardArtifactAudit.nTargetSamples, ...
            rewardArtifactAudit.nInterpolated, ...
            rewardArtifactAudit.nRemainingNaN);
    end

    % Keep a thresholded display copy. Spatial acceleration uses the
    % corrected, unthresholded speed above.
    ...
    P.fileName = file{f};
    P.rewardArtifactAudit = rewardArtifactAudit;
