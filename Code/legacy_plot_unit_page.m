function legacy_plot_unit_page(fig, unit, clusterID, rate_mean_halls, raster_data, halls, ...
    Blocks, tt_counts, binCenters, xMin, xMax, sc_all, metadataTable, opt)
...
% Rows 2+: Rate curves per block (with scene labels)
axRates = plot_rate_curves_legacy(tl, rate_mean_halls, unit, Blocks, tt_counts, binCenters, xMin, xMax, nBlocks, opt);
...
function axRates = plot_rate_curves_legacy(tl, rate_mean_halls, unit, Blocks, tt_counts, binCenters, xMin, xMax, nBlocks, opt)
...
for b = 1:nBlocks
    ...
    try
        ...
        if mod(b,2)==1, labelMap(3)='Novel C'; labelMap(4)='Novel D'; else, labelMap(5)='Swap C'; labelMap(6)='Swap D'; end
