function [FastSummaryWide, UnfSummaryWide] = block_filter()
% BLOCK_FILTER (multi-animal)
...
%% --- loop animals (parallel over animals) ---
nA = numel(roots);
FS_cells = cell(nA,1);
US_cells = cell(nA,1);
...
parfor aidx = 1:nA
    root = roots{aidx};
    [FS_cells{aidx}, US_cells{aidx}] = process_one_animal(root);
end

% --- accumulate to outputs (across animals) ---
FastSummaryWide = vertcat(FS_cells{:});
UnfSummaryWide  = vertcat(US_cells{:});

%% --- add 14th flag column based on column 13 threshold (5.5) ---
th = 5.5;  % threshold on Ratio_max_to_min_3blocks
FastSummaryWide.Fail = strings(height(FastSummaryWide),1);
ixF = FastSummaryWide.Ratio_max_to_min_3blocks > th;
FastSummaryWide.Fail(ixF) = "fail";

UnfSummaryWide.Fail = strings(height(UnfSummaryWide),1);
ixU = UnfSummaryWide.Ratio_max_to_min_3blocks > th;
UnfSummaryWide.Fail(ixU) = "fail";
...
%% -------- per-animal worker (used by parfor) --------
function [FastSummaryWide_one, UnfSummaryWide_one] = process_one_animal(root)
[~, animal] = fileparts(root);

% --- discover regions present (require at least one) ---
hasPPC = isfolder(fullfile(root,'PPC'));
hasVC  = isfolder(fullfile(root,'VC'));
if hasPPC && hasVC
    regions = {'PPC','VC'};
elseif hasPPC
    regions = {'PPC'};
elseif hasVC
    ...
    error('Neither PPC nor VC found in %s', root);
end

% per-animal collectors
...
for i = 1:numel(regions)
    ...
    if size(trialCounts,1) ~= nUnits
        error('trial_counts rows (%d) != number of units (%d)', size(trialCounts,1), nUnits);
    end
    if size(raster_data,1) ~= nUnits
        error('raster_data rows (%d) != number of units (%d)', size(raster_data,1), nUnits);
    end

    % ---- FAST: per unit per trial (preallocated) ----
    trials_per_block = cellfun(@numel, Blocks(:));
    total_rows_fast  = nUnits * sum(trials_per_block);
    f_rows = zeros(total_rows_fast, 4);   % [Block, Trial, UnitID, Count]
    wr = 1;
    for b = 1:3
        trList = Blocks{b}(:)';           % row vector of trial indices
        for tr = trList
            for u = 1:nUnits
                c = 0;
                C = raster_data{u,tr};
                if ~isempty(C) && isstruct(C) && isfield(C,'positions') && ~isempty(C.positions)
                    c = numel(C.positions);
                end
                f_rows(wr, :) = [b, tr, unitIDs(u), c];
                wr = wr + 1;
            end
        end
    end
    ...
    % ---- UNFILTERED: per unit per trial (preallocated) ----
    total_rows_unf = nUnits * sum(trials_per_block);
    u_rows = zeros(total_rows_unf, 4);    % [Block, Trial, UnitID, Count]
    wr = 1;
    for b = 1:3
        trList = Blocks{b}(:)';           % row vector of trial indices
        for tr = trList
            cr = trialCounts(:, tr);
            nr = numel(cr);
            u_rows(wr:wr+nr-1, :) = [repmat(b, nr, 1), repmat(tr, nr, 1), unitIDs, cr];
            wr = wr + nr;
        end
    end
